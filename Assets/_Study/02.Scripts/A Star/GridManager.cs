...
public class GridManager : MonoBehaviour
{
    ...
    private void CalculateObstacles()
    {
        ...
        for (int i = 0; i < rows; i++) // 모든 Grid에 Node 생성(맵핑)
        {
        ...
        if (obstacles.Length > 0) // Grid에 장애물 오브젝트가 있으면 해당 Node를 장애물로 설정
        {
    ...
    private Vector3 GetGridCellCenter(int index) // Grid의 중간 위치 확인하는 기능
    {
        ...
        //cellPos.z = (cellPos.z + gridCellSize) / 2;
    ...
    public bool isInBounds(Vector3 pos) // 특정 좌표가 Grid 범위 안에 있는지 확인하는 기능
    {
    ...
    public void GetNeighbors(Node node, List<Node> neighbors) // 현재 위치에서 주변 노드 검색하는 기능
    {
        ...
        AssignNegihbor(row - 1, col, neighbors);
        AssignNegihbor(row + 1, col, neighbors);
        AssignNegihbor(row, col + 1, neighbors);
        AssignNegihbor(row, col - 1, neighbors);

    }

    public void AssignNegihbor(int row, int col, List<Node> neighbors) // 해당 노드를 확인하는 기능
    {
